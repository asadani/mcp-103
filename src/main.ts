...
const db=await openDb(process.env.DATABASE_URL??'file://.data/teamspace-103'); await migrate(db); if(process.env.NODE_ENV!=='production'||process.env.SEED_DEMO_DATA==='1') await seed(db);
...
const server=createServer(app);server.listen(port,host,()=>console.log(`Teamspace 103 API and MCP: ${origin}\nUI development: npm run ui`));
