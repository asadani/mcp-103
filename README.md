...
- **The URL importer compares addresses numerically.** IPv4-mapped IPv6 is unwrapped first, IPv4 ranges include `100.64.0.0/10`, and IPv6 is an allow-list (global unicast only). The name is still resolved twice, so put an egress proxy or network policy in front for defence in depth.
